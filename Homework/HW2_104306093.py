...
def Q3(WAVE_NOW_FILENAME):
    ...
    for j in xrange(len(wave_data)):
        final_data = []
        for i in xrange(0, len(wave_data[0]), chunksize):
            chunk = wave_data[j][i:i+chunksize]
            fft_data = np.fft.rfft(chunk)
            new_fft = [fft_data[int(round(i/1.25))] for i in xrange(len(fft_data))]
            new_fft = np.fft.irfft(new_fft)
            final_data += new_fft.tolist()
            
        wave_data[j] = final_data
...
def Q4(WAVE_NOW_FILENAME):
    ...
    for i in range(wave_data.shape[0]):
        for j in range(10000,wave_data.shape[1]):
            wave_data[i][j] = wave_data[i][j] + wave_data[i][j-10000]*0.6
...
def Q5(WAVE_NOW_FILENAME):
    wave_data, time, params = wave_open(WAVE_NOW_FILENAME)
    wave_data[0] = wave_data[0][::-1]
    wave_data[1] = wave_data[1][::-1]
    wave_plot(wave_data,time)
